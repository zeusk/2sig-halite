...
while True:
    ...
    doomed = False
    if (nplayers == 4 and len(gmap.en_ships()) > len(gmap.my_ships())) and (len([e for e in gmap.unowned_planets() + gmap.my_uplanets() if e.remaining_resources > 0 ]) == 0):
        doomed = True
    ...
    for (s,e), d in move_list.items():
        ...
        if not doomed:
            ...
            if len(enemies):
                ...
                corners = []
                top_left = Position(Point(0,0))
                top_right = Position(Point(gmap.width,0))
                bottom_left = Position(Point(0,gmap.height))
                bottom_right = Position(Point(gmap.width,gmap.height))
                corners.append(top_left)
                corners.append(top_right)
                corners.append(bottom_left)
                corners.append(bottom_right)
                sorted(corners, key=lambda t:s.dist_to(t))
                pos = corners[0]
